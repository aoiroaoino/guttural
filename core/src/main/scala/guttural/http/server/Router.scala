...
object Router {
  ...
  def builder: DynamicBuilder = new DynamicBuilder(PartialFunction.empty)

  class DynamicBuilder(pf: PartialFunction[Request, Future[Response]]) {

    def andThen(other: PartialFunction[Request, Future[Response]]): DynamicBuilder =
      new DynamicBuilder(pf orElse other)

    def build: Router = ofDynamic(pf)
  }
  ...
  final case class Route(
    ...
    private[guttural] def isMatchingMethod(requestMethod: Method): Boolean =
      if (requestMethod == Method.HEAD) method == Method.HEAD || method == Method.GET
      else method == requestMethod

    private[guttural] def isMatchingPath(requestPath: String): Boolean =
